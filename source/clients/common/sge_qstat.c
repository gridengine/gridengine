   ...
   for_each(cqueue, qstat_env->queue_list) {
      ...
      for_each(qep, qinstance_list) {
         if ((lGetUlong(qep, QU_tag) & TAG_SHOW_IT)!=0) {
            ...
            handler->report_queue(handler, sge_dstring_get_string(&qinstance_name), alpp);
...
int qstat_cqueue_summary(qstat_env_t *qstat_env, cqueue_summary_handler_t *handler, lList **alpp) {
   ...
   for_each (cqueue, qstat_env->queue_list) {
      if (lGetUlong(cqueue, CQ_tag) != TAG_DEFAULT) {
         ...
         ret = handler->report_cqueue(handler, lGetString(cqueue, CQ_name), &summary, alpp);
         if(ret) {
            DRETURN(ret);
         }
      }
...
int qstat_no_group(qstat_env_t* qstat_env, qstat_handler_t* handler, lList **alpp) {
   ...
   if(handler->report_started) {
      if((ret = handler->report_started(handler, alpp))) {
    ...
    *
    */
    if( handler->report_zombie_jobs_started && (ret=handler->report_zombie_jobs_started(handler, alpp))) {
    ...
    if( handler->report_zombie_jobs_finished && (ret=handler->report_zombie_jobs_finished(handler, alpp))) {
       DPRINTF(("report_zombie_jobs_finished failed\n"));
       DRETURN(ret);
    }

   if(handler->report_finished) {
      if((ret = handler->report_finished(handler, alpp))) {
...
static int qstat_handle_running_jobs(qstat_env_t *qstat_env, qstat_handler_t *handler, lList **alpp) {
   ...
   for_each(qep, qstat_env->queue_list) {
      ...
      const char* queue_name = qinstance_get_name(qep, &queue_name_buffer);
...
static int qstat_env_get_all_lists(qstat_env_t* qstat_env, bool need_job_list, lList** alpp) {
...
#ifdef TEST_GDI2
   q_id = qstat_env->ctx->gdi_multi(qstat_env->ctx, &alp, SGE_GDI_RECORD, SGE_CQUEUE_LIST, SGE_GDI_GET,
...
#ifdef TEST_GDI2
      j_id = qstat_env->ctx->gdi_multi(qstat_env->ctx, &alp, SGE_GDI_RECORD, SGE_JOB_LIST, SGE_GDI_GET,
...
#ifdef TEST_GDI2
      z_id = qstat_env->ctx->gdi_multi(qstat_env->ctx, &alp, SGE_GDI_RECORD, SGE_ZOMBIE_LIST, SGE_GDI_GET, 
                           NULL, zw, qstat_get_JB_Type_filter(qstat_env), NULL, &state, true);
#else
...
#ifdef TEST_GDI2
   ce_id = qstat_env->ctx->gdi_multi(qstat_env->ctx, &alp, SGE_GDI_RECORD, SGE_CENTRY_LIST, SGE_GDI_GET, 
...
#ifdef TEST_GDI2
   eh_id = qstat_env->ctx->gdi_multi(qstat_env->ctx, &alp, SGE_GDI_RECORD, SGE_EXECHOST_LIST, SGE_GDI_GET,
...
#ifdef TEST_GDI2
      pe_id = qstat_env->ctx->gdi_multi(qstat_env->ctx, &alp, SGE_GDI_RECORD, SGE_PE_LIST, SGE_GDI_GET,
...
#ifdef TEST_GDI2
      ckpt_id = qstat_env->ctx->gdi_multi(qstat_env->ctx, &alp, SGE_GDI_RECORD, SGE_CKPT_LIST, SGE_GDI_GET,
...
#ifdef TEST_GDI2
      acl_id = qstat_env->ctx->gdi_multi(qstat_env->ctx, &alp, SGE_GDI_RECORD, SGE_USERSET_LIST, SGE_GDI_GET, 
...
#ifdef TEST_GDI2
      up_id = qstat_env->ctx->gdi_multi(qstat_env->ctx, &alp, SGE_GDI_RECORD, SGE_PROJECT_LIST, SGE_GDI_GET, 
...
#ifdef TEST_GDI2
   sc_id = qstat_env->ctx->gdi_multi(qstat_env->ctx, &alp, SGE_GDI_RECORD, SGE_SC_LIST, SGE_GDI_GET, 
...
#ifdef TEST_GDI2
   hgrp_id = qstat_env->ctx->gdi_multi(qstat_env->ctx, &alp, SGE_GDI_RECORD, SGE_HGROUP_LIST, SGE_GDI_GET, 
...
#ifdef TEST_GDI2
   gc_id = qstat_env->ctx->gdi_multi(qstat_env->ctx, &alp, SGE_GDI_SEND, SGE_CONFIG_LIST, SGE_GDI_GET,
   ...
   */
   ...
   if (!alp) {
      answer_list_add(alpp, MSG_GDI_QUEUESGEGDIFAILED, STATUS_EUNKNOWN, ANSWER_QUALITY_ERROR);
      DRETURN(1);
   }
   ...
   /* --- job */
   if (job_l) {
      sge_gdi_extract_answer(&alp, SGE_GDI_GET, SGE_JOB_LIST, j_id, mal, job_l);
...
#endif
      if (!alp) {
         answer_list_add(alpp, MSG_GDI_JOBSGEGDIFAILED, STATUS_EUNKNOWN, ANSWER_QUALITY_ERROR);
         DRETURN(1);
      }
      if (lGetUlong(lFirst(alp), AN_status) != STATUS_OK) {
         DRETURN(1);
   ...
   /* --- job zombies */
   if (zombie_l && show_zombies) {
      sge_gdi_extract_answer(&alp, SGE_GDI_GET, SGE_ZOMBIE_LIST, z_id, mal,
         zombie_l);
      if (!alp) {
         answer_list_add(alpp, MSG_GDI_JOBZOMBIESSGEGDIFAILED, STATUS_EUNKNOWN, ANSWER_QUALITY_ERROR);
         DRETURN(1);
      }
      if (lGetUlong(lFirst(alp), AN_status) != STATUS_OK) {
         DRETURN(1);
   ...
   sge_gdi_extract_answer(&alp, SGE_GDI_GET, SGE_CENTRY_LIST, ce_id,
                                mal, centry_l);
   if (!alp) {
      answer_list_add(alpp, MSG_GDI_COMPLEXSGEGDIFAILED, STATUS_EUNKNOWN, ANSWER_QUALITY_ERROR);
      DRETURN(1);
   }
   if (lGetUlong(lFirst(alp), AN_status) != STATUS_OK) {
      DRETURN(1);
   ...
   sge_gdi_extract_answer(&alp, SGE_GDI_GET, SGE_EXECHOST_LIST, eh_id, 
                                 mal, exechost_l);
   if (!alp) {
      answer_list_add(alpp, MSG_GDI_EXECHOSTSGEGDIFAILED, STATUS_EUNKNOWN, ANSWER_QUALITY_ERROR);
      DRETURN(1);
   }
   if (lGetUlong(lFirst(alp), AN_status) != STATUS_OK) {
      DRETURN(1);
   ...
   /* --- pe */
   if (pe_l) {
      sge_gdi_extract_answer(&alp, SGE_GDI_GET, SGE_PE_LIST, pe_id, 
                                    mal, pe_l);
      if (!alp) {
         answer_list_add(alpp, MSG_GDI_PESGEGDIFAILED, STATUS_EUNKNOWN, ANSWER_QUALITY_ERROR);
         DRETURN(1);
      }
      if (lGetUlong(lFirst(alp), AN_status) != STATUS_OK) {
         DRETURN(1);
   ...
   /* --- ckpt */
   if (ckpt_l) {
      sge_gdi_extract_answer(&alp, SGE_GDI_GET, SGE_CKPT_LIST, ckpt_id, 
                                    mal, ckpt_l);
      if (!alp) {
         answer_list_add(alpp, MSG_GDI_CKPTSGEGDIFAILED, STATUS_EUNKNOWN, ANSWER_QUALITY_ERROR);
         DRETURN(1);
      }
      if (lGetUlong(lFirst(alp), AN_status) != STATUS_OK) {
         DRETURN(1);
   ...
   /* --- acl */
   if (acl_l) {
      sge_gdi_extract_answer(&alp, SGE_GDI_GET, SGE_USERSET_LIST, acl_id, 
                                    mal, acl_l);
      if (!alp) {
         answer_list_add(alpp, MSG_GDI_PROJECTSGEGDIFAILED, STATUS_EUNKNOWN, ANSWER_QUALITY_ERROR);
         DRETURN(1);
      }
      if (lGetUlong(lFirst(alp), AN_status) != STATUS_OK) {
         DRETURN(1);
   ...
   /* --- project */
   if (project_l) {
      sge_gdi_extract_answer(&alp, SGE_GDI_GET, SGE_PROJECT_LIST, up_id, 
                                    mal, project_l);
      if (!alp) {
         answer_list_add(alpp, MSG_GDI_PROJECTSGEGDIFAILED, STATUS_EUNKNOWN, ANSWER_QUALITY_ERROR);
         DRETURN(1);
   ...
   /* --- scheduler configuration */
   sge_gdi_extract_answer(&alp, SGE_GDI_GET, SGE_SC_LIST, sc_id, mal, sc_l);
   if (!alp) {
      answer_list_add(alpp, MSG_GDI_SCHEDDCONFIGSGEGDIFAILED, STATUS_EUNKNOWN, ANSWER_QUALITY_ERROR);
      DRETURN(1);
   }
   if (lGetUlong(lFirst(alp), AN_status) != STATUS_OK) {
      DRETURN(1);
   ...
   sge_gdi_extract_answer(&alp, SGE_GDI_GET, SGE_HGROUP_LIST, hgrp_id, mal, 
                                hgrp_l);
   if (!alp) {
      answer_list_add(alpp, MSG_GDI_HGRPCONFIGGDIFAILED, STATUS_EUNKNOWN, ANSWER_QUALITY_ERROR);
      DRETURN(1);
   }
   if (lGetUlong(lFirst(alp), AN_status) != STATUS_OK) {
      DRETURN(1);
   ...
   sge_gdi_extract_answer(&alp, SGE_GDI_GET, SGE_CONFIG_LIST, gc_id, mal, &conf_l);
   if (!alp) {
      answer_list_add(alpp, MSG_GDI_GLOBALCONFIGGDIFAILED, STATUS_EUNKNOWN, ANSWER_QUALITY_ERROR);
      DRETURN(1);
   }
   if (lGetUlong(lFirst(alp), AN_status) != STATUS_OK) {
      DRETURN(1);
   ...
   while ((jep=nxt)) {
      ...
      while((jatep = nxt_jatep)) { 
         ...
         if (!(lGetUlong(jatep, JAT_suitable) & TAG_FOUND_IT) && 
            ...
            if ((!lGetNumberOfElem(qstat_env->user_list) || 
               ...
               if( handler->report_pending_jobs_started && (ret=handler->report_pending_jobs_started(handler, alpp))) {
      ...
      if (jep != nxt && qstat_env->full_listing & QSTAT_DISPLAY_PENDING) {
         ret = handle_jobs_not_enrolled(jep, NULL, true, NULL,
                                        0, 0, qstat_env, handler, alpp);
      }
   }
   
   if (ret == 0 && count > 0) {
      if( handler->report_pending_jobs_finished && (ret=handler->report_pending_jobs_finished(handler, alpp))) {
...
static int sge_handle_job(lListElem *job, lListElem *jatep, lListElem *qep,
   ...
   if (qep != NULL) {
      summary.queue = qinstance_get_name(qep, &queue_name_buffer);
   } else {
      summary.queue = NULL;
   }
