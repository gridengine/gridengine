...
public class GEObjectEditor {
   ...
   private static void updatePropertyValue(JGDI jgdi, GEObject obj, Object key, String values) {
      ...
      //For CONFIGURATION objects
         ...
         //TODO LP: Find out expected behaviour!
   ...
   public static String getPropertiesAsText(GEObject obj, int propScope) {
      ...
      //This is ssconf formatting special case
      ...
      for (Iterator iter = getProperties(obj, propScope).iterator(); iter.hasNext();) {
         ...
         if (subNames.size() > 0) {
            ...
            for (int j=0; j < subNames.size(); j++ ) {
               ...
               //TODO LP: Why -sconf adds 3 spaces after the value and prints one new line at the end?
               sb.append("    "+value+"\n");
            }
         } else {
            name = EditorUtil.java2cName(obj, pd.getPropertyName());
            value = EditorUtil.translateObjectToStringValue(pd.getPropertyName(), EditorUtil.getPropertyValue(obj, pd));
            //CONFIGURATION EXCEPTION for hname
            if (obj instanceof ConfigurationImpl && name.equals("hname")) {
               sb.append(value+":\n");
               continue;
            }
            ...
            if (obj instanceof UserSet) {
               ...
               spaces = " ";
               while (name.length() + spaces.length() < 30) {
                  spaces += " ";
               }
            } else if (name.equals("max_functional_jobs_to_schedule")) {
               spaces = "   ";
            } else {
               spaces = "    ";
            }
