...
CKernelObject::CKernelObject(const CKernelObject& KernelObject) :
   CHandle()
{
   ...
   bRet = DuplicateHandle(hPseudoHandle, KernelObject.GetHandle(), hPseudoHandle, 
      &hNewHandle, 0, FALSE, DUPLICATE_SAME_ACCESS); 
   if (bRet == 0) {
      throw CCodineException(CError::GetErrorMessage(GetLastError()), __FILE__, __LINE__);
   } else {
      SetHandle(hNewHandle);
   }
}
