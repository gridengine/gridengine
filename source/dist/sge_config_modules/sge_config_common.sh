...
BasicSettings()
{
  ...
  #SGE_COMMD_NAME=sge_commd
  #SGE_COMMDCNTL_NAME=sgecommdcntl
...
"
  if [ $FILE != "undef" ]; then
     ...
     $INFOTEXT -log "Reading configuration from default file sge.conf"
     #ReadConfigFile ./sge_config_modules/sge.conf
     . ./sge_config_modules/sge.conf
  fi
  IFS="   
"
}
...
#--------------------------------------------------------------------------
# Read Configuration from a SGE Configuration File
# Args: configuration filename
#
ReadConfigFile()
{
  config=$1
  ...
  if [ -f $config ]; then
     ...
     $INFOTEXT -log "Can't find configuration file, please enter the valid path"
     exit 1
  fi
  ...
  for line in `cat $config`; do 
   ...
   case $line in
      ...
      "")
         ...
         ;;
       *)
         var=`eval echo $line | cut -d "=" -f1`
         value=`eval echo $line | cut -d "=" -f2`

         if [ $var = "SGE_ROOT" ]; then
          SGE_ROOT=$value
         fi

         if [ $var = "GID_RANGE" ]; then
          GID_RANGE=$value
         fi
   
         if [ $var = "SGE_QMASTER_PORT" ]; then
          SGE_QMASTER_PORT=$value
         fi
          
         if [ $var = "SGE_EXECD_PORT" ]; then
          SGE_EXECD_PORT=$value
         fi

         if [ $var = "COMMD_PORT" ]; then
          COMMD_PORT=$value
         fi

         if [ $var = "ADD_PORT" ]; then
          ADD_PORT=$value
         fi

         if [ $var = "SGE_CELL_ROOT" ]; then
          SGE_CELL_ROOT=$value
         fi

         if [ $var = "CELL_NAME" ]; then
          CELL_NAME=$value
         fi

         if [ $var = "QMASTER_SPOOL_DIR" ]; then
          QMASTER_SPOOL_DIR=$value
         fi

         if [ $var = "HOSTNAME_RESOLVING" ]; then
          HOSTNAME_RESOLVING=$value
         fi

         if [ $var = "EXECD_SPOOL_DIR" ]; then
          EXECD_SPOOL_DIR=$value
         fi

         if [ $var = "ADMIN_MAIL" ]; then
          ADMIN_MAIL=$value
         fi

         if [ $var = "ADD_TO_RC" ]; then
          ADD_TO_RC=$value
         fi

         if [ $var = "RESCHEDULE_JOBS" ]; then
          RESCHEDULE_JOBS=$value
         fi

         if [ $var = "ADMIN_HOST_LIST" ]; then
          ADMIN_HOST_LIST=$value
         fi

         if [ $var = "SUBMIT_HOST_LIST" ]; then
          SUBMIT_HOST_LIST=$value
         fi

         if [ $var = "EXEC_HOST_LIST" ]; then
          EXEC_HOST_LIST=$value
         fi

         if [ $var = "SHADOW_HOST" ]; then
          SHADOW_HOST=$value
         fi

         if [ $var = "DB_SPOOLING_SERVER" ]; then
          DB_SPOOLING_SERVER=$value
         fi

         if [ $var = "DB_SPOOLING_DIR" ]; then
          DB_SPOOLING_DIR=$value
         fi

         if [ $var = "START_RPC_SERVICE" ]; then
          START_RPC_SERVICE=$value
         fi
   esac      
  done 
}
...
#-------------------------------------------------------------------------
# CheckCommPorts: checking, if a communication service already exists
#
CheckCommPorts()
{
  hosttype=$1
...
if [ $hosttype = "qmaster" ]; then
   $SGE_UTILBIN/getservbyname $SGE_MASTER_SRV > /dev/null 2>&1 
   ...
   if [ "$SGE_QMASTER_PORT" != "" ]; then
      ...
      if [ $SGE_QMASTER_PORT -ge 1 -a $SGE_QMASTER_PORT -le $port_max ]; then
         ...
         $INFOTEXT "\nThe environment variable\n\n" \
                   ...
                   "the installation or configure the service >sge_qmaster<." $SGE_QMASTER_PORT $port_max
   ...
   if [ $ret != 0 ]; then
      ...
      while [ $done = false ]; do
         $SGE_UTILBIN/getservbyname $SGE_MASTER_SRV 2>/dev/null
         if [ $? != 0 ]; then
            $CLEAR
            ...
            if [ $? = 0 ]; then
               ...
               elif [ $INP -le 1 -o $INP -ge $port_max ]; then
                  $INFOTEXT "\nInvalid port number. Must be in range [1..%s]." $port_max
   ...
   if [ "$SGE_EXECD_PORT" != "" ]; then
      ...
      if [ $SGE_EXECD_PORT -ge 1 -a $SGE_EXECD_PORT -le $port_max ]; then
         $INFOTEXT "\nUsing the environment variable\n\n" \
                   "   \$SGE_EXECD_PORT=%s\n\n" \
                     "as port for communication with >sge_execd<.\n\n" $SGE_EXECD_PORT
                      export SGE_EXECD_PORT
                      $INFOTEXT -log "Using port >%s<." $SGE_EXECD_PORT
         if [ $ret = 0 ]; then
         ...
         $INFOTEXT "\nThe environment variable\n\n" \
                   ...
                   "the installation or configure the service >sge_execd<." $SGE_EXECD_PORT $port_max
