   ...
   for_each(event, event_list) {
      switch(lGetUlong(event, ET_type)) {
         ...
         case sgeE_JATASK_DEL:
            job_id = lGetUlong(event, ET_intkey);
            task_id = lGetUlong(event, ET_intkey2);
            qevent_trigger_scripts(QEVENT_JB_TASK_END, option_struct, event);
            break;
         default:
            break;
      }
...
int main(int argc, char **argv)
{
   ...
   /* here we can implement more functionality */


   if (enabled_options.trigger_option_count > 0) {
      /* put out information about -trigger option */   

      ec_prepare_registration(EV_ID_ANY, "qevent -trigger");
      ...
      while(!shut_me_down) {
         ...
         if(event_list != NULL) {
            analyze_eventlist(event_list,&enabled_options);
