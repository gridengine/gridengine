...
/* NEED A LOCK FROM HERE ON */
   {
      ...
      */
      ...
      if (lGetUlong(jep, JB_verify_suitable_queues) != JUST_VERIFY &&
          lGetUlong(jep, JB_verify_suitable_queues) != PROBE_VERIFY) {
      ...
      */
      ...
      /* verify schedulability */
      {
         ...
         if (lGetUlong(jep, JB_verify_suitable_queues)==JUST_VERIFY  || 
             lGetUlong(jep, JB_verify_suitable_queues)==PROBE_VERIFY || ret != 0) {
            SGE_UNLOCK(LOCK_GLOBAL, LOCK_WRITE);
            DRETURN(ret);
         }   
      }
...
/*      u_long32 succ_jid;*/
   ...
   if ((pos=lGetPosViaElem(jep, JB_verify_suitable_queues, SGE_NO_ABORT))>=0) {
      ...
      if (lGetUlong(new_job, JB_verify_suitable_queues)==JUST_VERIFY || 
          lGetUlong(new_job, JB_verify_suitable_queues)==PROBE_VERIFY || ret != 0) {
         DRETURN(ret);
      }   
   }
...
static int verify_suitable_queues(lList **alpp, lListElem *jep, int *trigger, bool is_modify)
{
   ...
   bool verify_only = (verify_mode == JUST_VERIFY || verify_mode == PROBE_VERIFY) ? true : false;
...

   {
      ...
      if (!try_it || !ngranted) {
         ...
         SGE_ADD_MSG_ID(sprintf(SGE_EVENT, MSG_JOB_NOSUITABLEQ_S,
            (verify_only? MSG_JOB_VERIFYVERIFY: 
