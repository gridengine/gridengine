...
static void ec_add_subscriptionElement(sge_evc_class_t *thiz, lListElem *event_el, ev_event event, bool flush, int interval); 
static void ec_remove_subscriptionElement(sge_evc_class_t *thiz, lListElem *event_el, ev_event event); 
static void ec_mod_subscription_flush(sge_evc_class_t *thiz, lListElem *event_el, ev_event event, bool flush, int intervall);
static void ec_config_changed(sge_evc_class_t *thiz);
...
static bool ec_is_initialized(sge_evc_class_t *thiz); 
static void ec_mark4registration(sge_evc_class_t *thiz);
static bool ec_need_new_registration(sge_evc_class_t *thiz);
static int ec_set_edtime(sge_evc_class_t *thiz, int interval);
static int ec_get_edtime(sge_evc_class_t *thiz);
static bool ec_set_flush_delay(sge_evc_class_t *thiz, int flush_delay);
static int ec_get_flush_delay(sge_evc_class_t *thiz);
static bool ec_set_busy_handling(sge_evc_class_t *thiz, ev_busy_handling handling);
static ev_busy_handling ec_get_busy_handling(sge_evc_class_t *thiz);
static bool ec_register(sge_evc_class_t *thiz, bool exit_on_qmaster_down, lList** alpp);
static bool ec_deregister(sge_evc_class_t *thiz);
static bool ec_subscribe(sge_evc_class_t *thiz, ev_event event);
static void ec_add_subscriptionElement(sge_evc_class_t *thiz, lListElem *event_el, ev_event event, bool flush, int interval);
static void ec_mod_subscription_flush(sge_evc_class_t *thiz, lListElem *event_el, ev_event event, bool flush, int intervall);
static bool ec_mod_subscription_where(sge_evc_class_t *thiz, ev_event event, const lListElem *what, const lListElem *where);
static void ec_remove_subscriptionElement(sge_evc_class_t *thiz, lListElem *event_el, ev_event event);
static bool ec_subscribe_all(sge_evc_class_t *thiz);
static bool ec_unsubscribe(sge_evc_class_t *thiz, ev_event event);
static bool ec_unsubscribe_all(sge_evc_class_t *thiz);
static int ec_get_flush(sge_evc_class_t *thiz, ev_event event);
static bool ec_set_flush(sge_evc_class_t *thiz, ev_event event, bool flush, int interval);
static bool ec_unset_flush(sge_evc_class_t *thiz, ev_event event);
static bool ec_subscribe_flush(sge_evc_class_t *thiz, ev_event event, int flush);
static bool ec_set_busy(sge_evc_class_t *thiz, int busy);
static bool ec_get_busy(sge_evc_class_t *thiz);
static bool ec_set_session(sge_evc_class_t *thiz, const char *session);
static const char *ec_get_session(sge_evc_class_t *thiz);
static void ec_config_changed(sge_evc_class_t *thiz);
static bool ec_commit(sge_evc_class_t *thiz);
static bool ec_commit_multi(sge_evc_class_t *thiz, lList **malpp, state_gdi_multi *state);
static bool ec_get(sge_evc_class_t *thiz, lList **event_list, bool exit_on_qmaster_down);
static bool get_event_list(sge_evc_class_t *thiz, int sync, lList **report_list, int *commlib_error);
static ev_registration_id ec_get_id(sge_evc_class_t *thiz);
...
sge_evc_class_t *sge_evc_class_create(sge_gdi_ctx_class_t *sge_gdi_ctx, ev_registration_id reg_id, sge_error_class_t *eh)
{
   sge_evc_class_t *ret = (sge_evc_class_t *)sge_malloc(sizeof(sge_evc_class_t));
   sge_evc_t *sge_evc = NULL;

   DENTER(GDI_LAYER, "sge_evc_class_create");
   ...
   ret->ec_register = ec_register;
   ret->ec_deregister = ec_deregister;
   ret->ec_is_initialized = ec_is_initialized;
   ret->ec_subscribe = ec_subscribe;
   ret->ec_subscribe_all = ec_subscribe_all;
   ret->ec_unsubscribe = ec_unsubscribe;
   ret->ec_unsubscribe_all = ec_unsubscribe_all;
   ret->ec_get_flush = ec_get_flush;
   ret->ec_set_flush = ec_set_flush;
   ret->ec_unset_flush = ec_unset_flush;
   ret->ec_subscribe_flush = ec_subscribe_flush;
   ret->ec_mod_subscription_where = ec_mod_subscription_where;
   ret->ec_set_edtime = ec_set_edtime;
   ret->ec_get_edtime = ec_get_edtime;
   ret->ec_set_busy_handling = ec_set_busy_handling;
   ret->ec_get_busy_handling = ec_get_busy_handling;
   ret->ec_set_flush_delay = ec_set_flush_delay;
   ret->ec_get_flush_delay = ec_get_flush_delay;
   ret->ec_set_busy = ec_set_busy;
   ret->ec_get_busy = ec_get_busy;
   ret->ec_set_session = ec_set_session;
   ret->ec_get_session = ec_get_session;
   ret->ec_get_id = ec_get_id;
   ret->ec_commit = ec_commit;
   ret->ec_commit_multi = ec_commit_multi;
   ret->ec_get = ec_get;
   ret->ec_mark4registration = ec_mark4registration;
   ret->ec_need_new_registration = ec_need_new_registration;
   ...
   if (!sge_evc_setup(ret, sge_gdi_ctx, reg_id, eh)) {
      sge_evc_class_destroy(&ret);
      DEXIT;
      return NULL;
   }
...
{
   DENTER(GDI_LAYER, "sge_evc_class_destroy");
...
static void sge_evc_destroy(sge_evc_t **sge_evc)
{
   DENTER(TOP_LAYER, "sge_evc_destroy");
                          ...
                          sge_error_class_t *eh)
{
   ...
   DENTER(TOP_LAYER, "sge_evc_setup");
   ...
   if (id >= EV_ID_FIRST_DYNAMIC || name == NULL || *name == 0) {
      ...
      if (sge_evc->ec != NULL) {
         ...
         ec_subscribe_flush(thiz, sgeE_QMASTER_GOES_DOWN, 0);
         ec_subscribe_flush(thiz, sgeE_SHUTDOWN, 0);

         ec_set_busy_handling(thiz, EV_BUSY_UNTIL_ACK);
         lSetUlong(sge_evc->ec, EV_busy, 0);
         ec_config_changed(thiz);
         ret = true;
      }
   }
...
static void ec_mark4registration(sge_evc_class_t *thiz)
{
   ...
   const char *mastername = sge_gdi_ctx->get_master(sge_gdi_ctx);
   const char *progname = sge_gdi_ctx->get_progname(sge_gdi_ctx);
...
static bool ec_deregister(sge_evc_class_t *thiz)
{
   ...
   if (sge_evc->ec != NULL) {
      ...
      if (init_packbuffer(&pb, sizeof(u_long32), 0) == PACK_SUCCESS) {
         ...
         send_ret = sge_gdi2_send_any_request(0, NULL, sge_gdi_ctx, &pb, TAG_EVENT_CLIENT_EXIT, 0, &alp);
...
static bool ec_subscribe(sge_evc_class_t *thiz, ev_event event)
{
   ...
   if (event == sgeE_ALL_EVENTS) {
      ...
      for(i = sgeE_ALL_EVENTS; i < sgeE_EVENTSIZE; i++) {
         ec_add_subscriptionElement(thiz, sge_evc->ec, i, EV_NOT_FLUSHED, -1);
      }
   } else {
      ec_add_subscriptionElement(thiz, sge_evc->ec, event, EV_NOT_FLUSHED, -1);
   }
...
static bool ec_subscribe_all(sge_evc_class_t *thiz)
{
   return ec_subscribe(thiz, sgeE_ALL_EVENTS);
}
...
static bool ec_unsubscribe(sge_evc_class_t *thiz, ev_event event)
{
   ...
   if (event == sgeE_ALL_EVENTS) {
      ev_event i;
      for(i = sgeE_ALL_EVENTS; i < sgeE_EVENTSIZE; i++) {
         ec_remove_subscriptionElement(thiz, sge_evc->ec, i);
      }
      ec_add_subscriptionElement(thiz, sge_evc->ec, sgeE_QMASTER_GOES_DOWN, EV_FLUSHED, 0);
      ec_add_subscriptionElement(thiz, sge_evc->ec, sgeE_SHUTDOWN, EV_FLUSHED, 0);

   } else {
      if (event == sgeE_QMASTER_GOES_DOWN || event == sgeE_SHUTDOWN) {
         ERROR((SGE_EVENT, MSG_EVENT_HAVETOHANDLEEVENTS));
      } else {
         ec_remove_subscriptionElement(thiz, sge_evc->ec, event);
      }
   }
...
static bool ec_unsubscribe_all(sge_evc_class_t *thiz)
{
   return ec_unsubscribe(thiz, sgeE_ALL_EVENTS);
}
...
static bool ec_set_flush(sge_evc_class_t *thiz, ev_event event, bool flush, int interval)
{
   ...
   if (sge_evc->ec == NULL) {
      ...
      if (!flush ) {
         PROF_STOP_MEASUREMENT(SGE_PROF_EVENTCLIENT);
         ret = ec_unset_flush(thiz, event);
         ec_mod_subscription_flush(thiz, sge_evc->ec, event, EV_NOT_FLUSHED, EV_NO_FLUSH);
         PROF_START_MEASUREMENT(SGE_PROF_EVENTCLIENT);
/*      } else if (interval < 0 || interval > EV_MAX_FLUSH) {
         ...
         if (sub_event == NULL) {
            ...
            ec_mod_subscription_flush(thiz, sge_evc->ec, event, EV_FLUSHED, interval);
         }
         if (lGetBool(sge_evc->ec, EV_changed)) {
            ec_config_changed(thiz);
            ret = true;
...
static bool ec_subscribe_flush(sge_evc_class_t *thiz, ev_event event, int flush) 
{
   ...
   ret = ec_subscribe(thiz, event);
   if (ret) {
      if (flush >= 0)
         ret = ec_set_flush(thiz, event, true, flush);
      else
         ret = ec_set_flush(thiz, event, false, flush);
   }
...
static ev_registration_id ec_get_id(sge_evc_class_t *thiz)
{
   ...
   DEXIT;
   return (ev_registration_id)lGetUlong(sge_evc->ec, EV_id);
}
...
static bool ec_commit(sge_evc_class_t *thiz) 
{
   ...
   } else if (ec_need_new_registration(thiz)) {
...
static bool ec_get(sge_evc_class_t *thiz, lList **event_list, bool exit_on_qmaster_down) 
{
   ...
   if (sge_evc->ec == NULL) {
      ...
      sge_evc->next_event = 1;
      ret = ec_register(thiz, exit_on_qmaster_down, NULL);
   }
   ...
   if (ret) {
      ...
      if (last_fetch_ok_time == 0) {
         last_fetch_ok_time = ec_get_edtime(thiz);
      }

      /* initialize the maximum number of fetches */
      if (last_fetch_time == 0) {
         max_fetch = ec_get_edtime(thiz);
      } else {
      ...
      if (sync && !fetch_ok) {
         u_long32 timeout = ec_get_edtime(thiz) * 10;
...
static bool get_event_list(sge_evc_class_t *thiz, int sync, lList **report_list, int *commlib_error )
{
   ...
   if ( (help=sge_gdi2_get_any_request(sge_gdi_ctx, &pb, &tag, sync,0,NULL)) != CL_RETVAL_OK) {
