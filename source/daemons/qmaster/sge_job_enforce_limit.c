...
*
*     When the event is triggered the function sge_job_enforce_limit_handler()
*     is executed.
*
...
*
*  SEE ALSO
*     qmaster/qmaster-execd/sge_job_enforce_limit_handler
...
sge_host_add_remove_enforce_limit_trigger(const char *hostname, bool add) 
{
   ...
   if (is_module_enabled()) {
      ...
      for_each (job, *(object_type_get_master_list(SGE_TYPE_JOB))) {
         for_each (ja_task, lGetList(job, JB_ja_tasks)) {
            ...
            if (gdil_ep != NULL) { 
               ...
               /*
                * Either we got a hostname. Then we can add triggers for jobs running on that host.
      ...
      INFO((SGE_EVENT, MSG_JOB_ADDDELLIMITTOOK_U, sge_u32c(end - start)));
   }
   DRETURN_VOID;
}
...
/****** qmaster/qmaster-execd/sge_job_enforce_limit_handler() *******************
*  NAME
*     sge_job_enforce_limit_handler() -- enforces wallclock limits in master 
*
*  SYNOPSIS
*     void 
*     sge_job_enforce_limit_handler(sge_gdi_ctx_class_t *ctx, 
...
*
*  NOTES
*     MT-NOTE: sge_job_enforce_limit_handler() is MT safe 
*
*  SEE ALSO
*     qmaster/qmaster-execd/sge_host_add_remove_enforce_limit_trigger() 
*     qmaster/qmaster-execd/sge_host_add_enforce_limit_trigger() 
...
{
   DENTER(TOP_LAYER, "sge_job_enforce_limit_handler");
...
*     h_rt or s_rt limit of the job or of one of the pe tasks part of the
*     job is reached. 
*
   ...
   if (is_module_enabled()) {
      ...
      if (job != NULL && ja_task != NULL) {
          ...
          */
         {
            ...
            /*
             * Calculate how long the job still might run till it has to
             * be terminated because of exceeding the wallclock limits
             */
            {
               u_long32 job_s_rt = U_LONG32_MAX;
               u_long32 qi_s_rt = U_LONG32_MAX;
                ...
                */
               {
                  ...
                  lListElem *cple = lGetElemStr(cplxl, CE_name, SGE_ATTR_S_RT);

                  if (cple != NULL) {
                     const char *job_limit = lGetString(cple, CE_stringval);

                     if (job_limit != NULL && strcasecmp(job_limit, "infinity") != 0) {
                        parse_ulong_val(NULL, &job_s_rt, TYPE_TIM, job_limit, NULL, 0);
                        has_rt_limit = true;
                     }
                  }
               }
               ...
               if (has_rt_limit == false) {
                  lList *gdil = lGetList(ja_task, JAT_granted_destin_identifier_list);
                  u_long32 current_qi_s_rt = U_LONG32_MAX;
                  lListElem *gdil_ep;
                  ...
                  for_each(gdil_ep, gdil) {
                     ...
                     if (qi != NULL) {
                        const char *qi_limit = lGetString(qi, QU_s_rt);

                        if (qi_limit != NULL && strcasecmp(qi_limit, "infinity") != 0) {
                           parse_ulong_val(NULL, &current_qi_s_rt, TYPE_TIM, qi_limit, NULL, 0);
                           has_rt_limit = true;
                           qi_s_rt = MIN(current_qi_s_rt, qi_s_rt);
                        } 
                     }
                  }
               }
               ...
               max_running = MIN(MIN(qi_s_rt, job_s_rt), MIN(qi_h_rt, job_h_rt)); 
