...
static void
sge_deliverer_cleanup_monitor(monitoring_t *monitor)
{
   DENTER(TOP_LAYER, "sge_deliverer_cleanup_monitor");
   sge_monitor_free(monitor);
   DRETURN_VOID;
}

static void
sge_deliverer_cleanup_report_list(lList **list)
{
   DENTER(TOP_LAYER, "sge_deliverer_cleanup_monitor");
   lFreeList(list);
   DRETURN_VOID;
...
void 
sge_deliverer_initialize(sge_gdi_ctx_class_t *ctx)
{
   cl_thread_settings_t* dummy_thread_p = NULL;
   dstring thread_name = DSTRING_INIT;

   DENTER(TOP_LAYER, "sge_deliverer_initialize");
   ...
   cl_thread_list_setup(&(Main_Control.deliverer_thread_pool), "event deliverer thread pool");
   cl_thread_list_create_thread(Main_Control.deliverer_thread_pool, &dummy_thread_p,
                                NULL, sge_dstring_get_string(&thread_name), 0, 
                                sge_deliverer_main, NULL, NULL);
   sge_dstring_free(&thread_name);
   DRETURN_VOID;
}

void
sge_deliverer_terminate(void)
{
   cl_thread_settings_t* thread = NULL;

   DENTER(TOP_LAYER, "sge_deliverer_terminate");

   thread = cl_thread_list_get_first_thread(Main_Control.deliverer_thread_pool);
   while (thread != NULL) {
      DPRINTF((SFN" gets canceled\n", thread->thread_name));
      cl_thread_list_delete_thread(Main_Control.deliverer_thread_pool, thread);
      thread = cl_thread_list_get_first_thread(Main_Control.deliverer_thread_pool);
   }  
...
sge_deliverer_main(void *arg)
{
   ...
   DENTER(TOP_LAYER, "sge_deliverer_main");
   ...
   while (do_endlessly) {
      ...
      pthread_cleanup_push((void (*)(void *))sge_deliverer_cleanup_monitor,
                           (void *)&monitor);
      pthread_cleanup_push((void (*)(void *))sge_deliverer_cleanup_report_list,
                           (void *)&report_list);
