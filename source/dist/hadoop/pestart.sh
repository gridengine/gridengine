...
hostname=`hostname`
. $SGE_ROOT/hadoop/make_conf.sh $hostname
...
nohup "$bin"/slaves.sh --config $HADOOP_CONF_DIR "$SGE_ROOT/hadoop/make_conf.sh" $hostname \; "$bin/hadoop-daemon.sh" --config $HADOOP_CONF_DIR start tasktracker \; "$SGE_ROOT/hadoop/wait.sh" >> $logfile 2>&1 < /dev/null &
