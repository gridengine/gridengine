...
static bool limit_rule_set_provide_modify_context(sge_gdi_ctx_class_t *ctx,
...
bool limit_rule_show(sge_gdi_ctx_class_t *ctx, lList **answer_list, const char *name)
{
   ...
   if (name != NULL) {
      ...
      ret = limit_rule_get_via_gdi(ctx, answer_list, lirsref_list, &lirs_list);
      lFreeList(&lirsref_list);
   } else {
      ret = limit_rule_get_all_via_gdi(ctx, answer_list, &lirs_list);
   }
...
bool limit_rule_set_add(sge_gdi_ctx_class_t *ctx, lList **answer_list, const char *name)
{
   ...
   if (name != NULL) {
      ...
      ret = limit_rule_set_provide_modify_context(ctx, &lirs_list, answer_list, true);

      if (ret) {
         ret = limit_rule_set_add_del_mod_via_gdi(ctx, lirs_list, answer_list, SGE_GDI_ADD | SGE_GDI_SET_ALL);
      }
...
bool limit_rule_set_modify(sge_gdi_ctx_class_t *ctx, lList **answer_list, const char *name)
{
   ...
   if (ret) {
      ret = limit_rule_set_provide_modify_context(ctx, &lirs_list, answer_list, false);
   }
   if (ret) {
      ret = limit_rule_set_add_del_mod_via_gdi(ctx, lirs_list, answer_list,
                                       gdi_command);
   }
...
*
*******************************************************************************/
static bool limit_rule_set_provide_modify_context(sge_gdi_ctx_class_t *ctx, lList **lirs_list, lList **answer_list,
                                           bool ignore_unchanged_message)
{
   bool ret = false;
   int status = 0;
