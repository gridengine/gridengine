...
%>           
   ...
   public boolean isSet<%=gsname%>(String <%=mapAttr.getKeyName()%>) {
      return m_<%=attrName%> != null &&
             m_<%=attrName%>.get(<%=mapAttr.getKeyName()%>) != null;
   }
...
<% for(int i = 0; i < cullObj.getPrimaryKeyCount(); i++ ) {
        ...
        String gsname =  Character.toUpperCase( attrName.charAt(0) ) +
                          attrName.substring(1);
%>
          // <%=attrName%>, type = <%=attr.getType()%>, isPrimitiv = <%=jh.isPrimitiv(attr)%><%
        if( jh.isPrimitiv(attr) ) { %>
          && get<%=gsname%>() == ((<%=jh.getFullClassName(cullObj)%>)obj).get<%=gsname%>() <%
        } else { %>
          && get<%=gsname%>().equals(((<%=jh.getFullClassName(cullObj)%>)obj).get<%=gsname%>()) <%
        } // end of isPrimitiv
    } %>
    ;
<%    
...
%>
            if (<%=value1%> == null ) {
               if (<%=value2%> != null) {
                  if(logger.isLoggable(Level.FINE)) {
                     logger.fine("attribute <%=attr.getName()%> of obj1 is not null");
                  }
                  return false;
               }
            } else {
...
%>
             if (!<%=value1%>.equals(<%=value2%>)) {
                if(logger.isLoggable(Level.FINE)) {
                   logger.fine("attribute <%=attr.getName()%> of obj1 is equals");
                }
                return false;
             }
<%                
