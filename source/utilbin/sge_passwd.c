...
read_private_key(const char *keyfile, char *ssl_err, size_t buff_size)
{
   ...
   if(keyfile == NULL) {
      snprintf(ssl_err, buff_size, MSG_PWD_FILE_PATH_NULL_S, prognames[SGE_PASSWD]);
      DEXIT;
      return NULL;
   }
   fp = fopen(keyfile, "r");
   if (!fp) {
      snprintf(ssl_err, buff_size, MSG_PWD_LOAD_PRIV_SSS, prognames[SGE_PASSWD], keyfile, MSG_PWD_NO_SSL_ERR);
      DEXIT;
      return NULL;
   }
   ...
   while (repeat) {
      ...
      if (buffer_length <= 0) {
         ...
         if (buffer_ptr != NULL) {
            ...
            fprintf(stderr, MSG_PWD_MALLOC_SS, prognames[SGE_PASSWD], MSG_PWD_NO_SSL_ERR);
            exit(1);
         }
      }
   }
               ...
               size_t *buffer_out_length)
{
   ...
   if(ret <= 0) {
      snprintf(err_str, MAX_STRING_SIZE, MSG_PWD_CANTLOADRANDFILE_SSS, 
              prognames[SGE_PASSWD], rand_file, MSG_PWD_NO_SSL_ERR);
...

{
   ...
   if (!privateKey) {
      snprintf(err_str, MAX_STRING_SIZE, MSG_PWD_LOAD_PRIV_SSS, 
              prognames[SGE_PASSWD], file_priv_key, err_msg);
#ifdef DEFINE_SGE_PASSWD_MAIN
      fprintf(stderr, err_str);
#endif
      DEXIT;
   ...
   if (ekeylen != shared_ssl_func__EVP_PKEY_size(privateKey)) {
      ...
      snprintf(err_str, MAX_STRING_SIZE, MSG_PWD_DECR_SS, prognames[SGE_PASSWD], err_msg);
#ifdef DEFINE_SGE_PASSWD_MAIN
      fprintf(stderr, "%s\n", err_str);
#endif
      DEXIT;
   ...
   if(ret <= 0) {
      ...
      ERR_error_string(error_code, err_msg);
      snprintf(err_str, MAX_STRING_SIZE, MSG_PWD_CANTLOADRANDFILE_SSS, 
              prognames[SGE_PASSWD], rand_file, err_msg);
   ...
   while (buffer_in_length > 0) {
      ...
      if (ret == 0) {
         error_code = ERR_get_error();
         ERR_error_string(error_code, err_msg);
         snprintf(err_str, MAX_STRING_SIZE, MSG_PWD_SSL_ERR_MSG_SS, prognames[SGE_PASSWD], err_msg);
#ifdef DEFINE_SGE_PASSWD_MAIN
         fprintf(stderr, "%s\n", err_str);
#endif
         DEXIT;
...
sge_passwd_delete(const char *username, const char *domain)
{
   ...
   if (password_read_file(&users, &encryped_pwd, sge_get_file_passwd()) == 2) {
      fprintf(stderr, MSG_PWD_FILE_CORRUPTED_S,prognames[SGE_PASSWD]);
      fprintf(stderr, "\n");
      exit(1);
   }
